    ...
    <div className="space-y-10 fade-in">
      <div className="flex justify-between items-end">
        <div className="space-y-1">
          <p className="p-engine-sm">Financial Suite</p>
          <p className="p-engine-xl text-(--text-main)">
            Project <span className="font-bold italic">Quotes.</span>
          </p>
        </div>
        <button className="bg-(--text-main) text-(--surface) px-6 py-3 rounded-xl flex items-center gap-3">
          <FilePlus2 size={16} />
      ...
      <div className="border border-(--border-color) rounded-xl overflow-hidden bg-(--surface)">
        ...
        {quotes.map((q) => (
          ...
          >
            <p className="text-xs font-bold font-mono">{q.id}</p>
            <p className="text-sm font-semibold tracking-tight">{q.client}</p>
            <p className="text-sm font-bold">${q.total}</p>
            <p className="text-[9px] font-black uppercase tracking-tighter text-right text-(--text-subtle)">
              {q.status}
            </p>
          </div>
        ))}
